...
cloud_image_path = "split_images/sun-cloud.jpgwebp"
...
# === Streamlit config ===
st.set_page_config(page_title="Digital Twin - TEST VERSION", layout="centered")
st.title("\U0001F6E0️ TEST MODE – Digital Twin Dashboard")
...
with tab1:
    if st.session_state.view == "home":
        st.markdown("### 🔍 Click on an icon for details")
...
with tab2:
    current_temp, current_desc, _ = get_weather("Perth")
    st.sidebar.title("🌤️ Weather")
    if current_temp:
        st.sidebar.write(f"{current_temp} °C, {current_desc.capitalize()}")

    selection = st.radio("Choose Data View", ["Temperature", "Humidity", "pH", "Soil Quality", "Event Logs"])
    ...
    if selection == "Temperature" and not df.empty:
        ...
        st.info("Feature under development (placeholder).")
...
# === EXTRA: TEST ZONA ===
st.markdown("---")
st.markdown("### 🧪 TEST  (for debug)")
if st.button("⬅️ Force Home View Reset"):
    st.session_state.view = "home"
st.write("📌 test version.")
